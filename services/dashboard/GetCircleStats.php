...
class GetCircleStats extends Connection
{
...
select f.circle_name as name, f.count as total, coalesce((g.count),null,0) as surveyed, f.count - coalesce(g.count, null,0)as unsurveyed, coalesce(un.count, null,0) as unassessed
from (
    select circle_name, count(*) as count from tbl_raw_data where district_name = (select d.district from d) and circle_name like (select d.circle from d) and landuse <> 'Others' group by circle_name
) as f left outer join (
